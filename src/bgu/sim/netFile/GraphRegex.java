...
/**
 * Created by IntelliJ IDEA. User: puzis Date: Aug 23, 2007 Time: 2:43:02 PM To
...
public class GraphRegex {
    ...
    public static Pattern ARCS_BEGINING = Pattern.compile("\\*Arcs");//@Sergei: added
    public static Pattern ARC_LINE = Pattern.compile("\\s*(\\d+)\\s+(\\d+)\\s+(\\d*[\\.]?\\d*)");//@Sergei: added
    public static Pattern SIMPLE_ARC_LINE = Pattern.compile("\\s*(\\d+)\\s+(\\d+)\\s*");
}
